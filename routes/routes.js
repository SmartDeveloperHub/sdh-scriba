...
module.exports = function(app) {
  ...
  app.get('/scriba', function(req, res) {
        console.log("/scriba ON")

        res.render('scriba'); // load view/scriba.html file
  });
  ...
  var saveUser = function(auth, identity){
    ...
    slack.controller.storage.owners.get(identity.user_id, function(err, user) {
      isnew = false;
      if (!user) {
          isnew = true;
          user = {
      ...
      slack.controller.storage.owners.save(user, function(err, id) {
        if (err) {
          console.log('An error occurred while saving a owner: ', err);
          slack.controller.trigger('error', [err]);
        }
        else {
          if (isnew) {
            console.log("New owner " + id.toString() + " saved");
          }
          else {
            console.log("Owner " + id.toString() + " updated");
          }
